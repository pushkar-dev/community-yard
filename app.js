...
mongoose.connect("mongodb+srv://" + process.env.username + ":" + process.env.pass + "@cs.ouhxb.mongodb.net/CS?retryWrites=true&w=majority", { useNewUrlParser: true }); //, {useNewUrlParser: true,useUnifiedTopology: true,}); //Running on localhost
...
passport.use(
    new GoogleStrategy({
            ...
            callbackURL: "https://community-yard.herokuapp.com/auth/google/CS",
...
app.post("/chatWithOwner", function(req, res) {
    const body = req.body;
    Chat.findOne({ item_name: body.item_name, owner_email: body.owner_email },
        function(err, found) {
            if (err) console.log(err);
            else {
                console.log(found);
...
app.post("/chatWithBuyer", function(req, res) {
    const body = req.body;
    Chat.findOne({ item_name: body.item_name, owner_email: req.user.email },
        function(err, found) {
            if (err) console.log(err);
            else {
                if (found) {
...
app.post("/deleteAd", function(req, res) {
    if (req.isAuthenticated()) {
        if (req.user.email === req.body.owner_email) {
            const body = req.body;
            Chat.deleteOne({ item_name: body.item_name, owner_email: body.owner_email },
                function(err, res) {
                    if (err) console.log(err);
                    else {
            ...
            Item.deleteOne({ item_name: body.item_name, owner_email: body.owner_email },
                function(err, res) {
                    if (err) console.log(err);
                    else {
